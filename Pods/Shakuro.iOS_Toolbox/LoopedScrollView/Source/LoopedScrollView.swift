...
public protocol LoopedScrollViewDataSource: class {

    /// - parameter loopedView: caller.
    /// - returns: number of elementes in looped scroll.
    ///     Negative value will throw assert (debug) or be rounded to zero (release).
    func loopedScrollViewNumberOfItems(_ loopedView: LoopedScrollView) -> Int
    ...
    ///     Height will be equal to the height of LoopedScrollView.
    func loopedScroll(_ loopedView: LoopedScrollView, widthForItemAtIndex index: Int) -> CGFloat
    ...
    /// Height will be always equal to height of LoopedScrollView itself.
    /// Consider creating etalon view and calculating dynamic width inside `loopedScroll(:,widthForItemAtIndex:)`.
    ///
    /// You can perform some additional configuration of returned view, depending on the provided size.
    func loopedScroll(_ loopedView: LoopedScrollView, viewForItemAtIndex index: Int, size: CGSize) -> UIView

}

public protocol LoopedScrollViewDelegate: class {

    /// Will not be reported if there are no items in LoopedScrollView
    func loopedScroll(_ loopedView: LoopedScrollView,
                      willEndSwipeOnItemAtIndex itemIndex: Int,
                      swipeDirection: LoopedScrollView.SwipeDirection)

    /// Will be reported even if deceleration is disabled.
    func loopedScroll(_ loopedView: LoopedScrollView, didEndDeceleratingOnItemAtIndex itemIndex: Int, wasAborted: Bool)

    /// User tapped specific item.
    /// Tap to stop deceleration animation do not count.
    func loopedScroll(_ loopedScroll: LoopedScrollView, didSelectItemAtIndex itemIndex: Int)

}

/// CollectView-like control. Handles looped collection of items.
...
public class LoopedScrollView: UIView {

    private enum Constant {
        static let snapAnimationKey: String = "LoopedScrollView.snapAnimation"
        static let snapAnimationNameValue: String = "LoopedScrollView.snapAnimation.name"
        static let snapAnimationNameKey: String = "LoopedScrollView.snapAnimation.key"
    }

    public weak var dataSource: LoopedScrollViewDataSource?
    public weak var delegate: LoopedScrollViewDelegate?
                ...
                if (cachedNumberOfItems > 1) || (singleItemBehavior == .tile) || isSnapEnabled {
                    ...
                    case .scrollView:
                        ...
                        if let itemIndex = visibleItem?.itemIndex {
                            delegate?.loopedScroll(self, didEndDeceleratingOnItemAtIndex: itemIndex, wasAborted: true)
                        }
                    case .curve, .spring:
            ...
            if isDecelerationEnabled {
                if (cachedNumberOfItems > 1) || (singleItemBehavior == .tile) || isSnapEnabled {
                    ...
                    case .none:
                        ...
                        // report
                        delegate?.loopedScroll(self, didEndDeceleratingOnItemAtIndex: decelerationData.nearestItemData.itemIndex, wasAborted: false)
                    case .scrollView:
                        // deceleration animation from UIScrollView is used
                ...
                if (cachedNumberOfItems > 1) || (singleItemBehavior == .tile) || isSnapEnabled {
                    ...
                    case .scrollView:
                        if let itemIndex = nearestVisibleCenterItem(targetOffsetX: scrollView.contentOffset.x)?.itemIndex {
                            delegate?.loopedScroll(self, didEndDeceleratingOnItemAtIndex: itemIndex, wasAborted: false)
                        }
                    ...
                    if let itemIndex = nearestVisibleCenterItem(targetOffsetX: scrollView.contentOffset.x)?.itemIndex {
                        ...
                        delegate?.loopedScroll(self, didEndDeceleratingOnItemAtIndex: itemIndex, wasAborted: scrollView.isTracking)
                    }
                    decelerationAnimationData = nil
                }
            } else {
    ...
    public func scrollViewWillEndDragging(_ scrollView: UIScrollView,
        ...
        if let nearestItemData = nearestVisibleCenterItem(targetOffsetX: projectedContentOffsetX) {
            if let startIndex = dragStartedTileIndex {
                ...
                delegate?.loopedScroll(self, willEndSwipeOnItemAtIndex: nearestItemData.itemIndex, swipeDirection: direction)
            }
            if isSnapEnabled {
...
extension LoopedScrollView: CAAnimationDelegate {
...
extension LoopedScrollView: UIGestureRecognizerDelegate {
...
private extension LoopedScrollView {
    ...
    private func recreateCacheFromDataSource() {
        ...
        cachedNumberOfItems = strongDataSource.loopedScrollViewNumberOfItems(self)
        ...
        for itemIndex in 0..<cachedNumberOfItems {
            let itemWidth = strongDataSource.loopedScroll(self, widthForItemAtIndex: itemIndex)
    ...
    private func tileItemViewsIfNeeded(allowThrottle: Bool, targetContentOffsetX: CGFloat?) {
        ...
        while currentTileX < maxVisibleX {
            ...
            if visibleTileViews[currentTileIndex] == nil {
                ...
                let newTileView = strongDataSource.loopedScroll(self, viewForItemAtIndex: currentItemIndex, size: newTileFrame.size)
    ...
    /// Move content so that center of content align with visual center of LoopedScrollView.
    ...
    private func setupSingleItemIfNeeded() {
        ...
        if let oldTileView = visibleTileViews[itemIndex] {
            ...
            let newTileView = strongDataSource.loopedScroll(self, viewForItemAtIndex: itemIndex, size: newTileFrame.size)
            newTileView.frame = newTileFrame
            contentContainerView.addSubview(newTileView)
    ...
    @objc private func handleTapGestureRecognized(_ gestureRecognizer: UIGestureRecognizer) {
        ...
        delegate?.loopedScroll(self, didSelectItemAtIndex: index)
    }
